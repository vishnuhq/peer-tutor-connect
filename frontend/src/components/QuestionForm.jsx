      ...
      <main id="main-content" className="container-centered fade-in" style={{ paddingTop: '2rem', paddingBottom: '2rem', maxWidth: '64rem' }}>
        ...
        <nav aria-label="Breadcrumb" style={{ marginBottom: '1.5rem' }}>
          <ol className="flex items-center" style={{ gap: '0.5rem', fontSize: '0.875rem' }}>
            ...
            <li>
              <Link to={`/courses/${courseId}/questions`} className="text-teal-600 hover:text-teal-700 hover:underline font-medium">
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p className="text-gray-600 text-base sm:text-lg" style={{ marginTop: '0.75rem' }}>
            {isEdit
        ...
        {/* Error Message */}
        {errorMessage && (
          <div className="bg-red-50 border border-red-200 text-red-800 flex items-start fade-in" style={{ gap: '0.75rem', borderRadius: '0.75rem', padding: '1rem', marginBottom: '1.5rem' }}>
            <AlertCircle style={{ width: '1.25rem', height: '1.25rem', flexShrink: 0 }} />
            <p style={{ fontSize: '0.875rem', fontWeight: 500 }}>{errorMessage}</p>
          </div>
        )}

        {/* Form */}
        <form onSubmit={handleSubmit} className="bg-white shadow-md border border-gray-100" style={{ borderRadius: '1rem', padding: '2.5rem' }}>
          {/* Title Field */}
          <div style={{ marginBottom: '2rem' }}>
            <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
              <label htmlFor="title" className="block font-semibold text-gray-900" style={{ fontSize: '1rem' }}>
                Title *
              </label>
              <span className={`font-medium ${title.length > 200 ? 'text-red-600' : 'text-gray-500'}`} style={{ fontSize: '0.875rem' }}>
                {title.length}/200
              </span>
            </div>
            <input
              ...
              style={{ padding: '0.875rem 1.25rem', borderRadius: '0.75rem', fontSize: '1rem' }}
              disabled={isSubmitting}
            />
            {errors.title && (
              <p className="text-red-600 font-medium" style={{ marginTop: '0.5rem', fontSize: '0.875rem' }} role="alert">
                {errors.title}
              </p>
            )}
          </div>
          ...
          <div style={{ marginBottom: '2rem' }}>
            <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
              <label htmlFor="content" className="block font-semibold text-gray-900" style={{ fontSize: '1rem' }}>
                Content *
              </label>
              <span className={`font-medium ${content.length > 2000 ? 'text-red-600' : 'text-gray-500'}`} style={{ fontSize: '0.875rem' }}>
                {content.length}/2000
              </span>
            </div>
            <textarea
              ...
              className={`w-full border focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all resize-y ${
                errors.content ? 'border-red-500' : 'border-gray-200 hover:border-gray-300'
              }`}
              style={{ padding: '0.875rem 1.25rem', borderRadius: '0.75rem', fontSize: '1rem', lineHeight: '1.6' }}
              disabled={isSubmitting}
            />
            {errors.content && (
              <p className="text-red-600 font-medium" style={{ marginTop: '0.5rem', fontSize: '0.875rem' }} role="alert">
          ...
          {!isEdit && (
            <div className="bg-gradient-to-r from-teal-50 to-emerald-50 border border-teal-100" style={{ marginBottom: '2rem', padding: '1.25rem', borderRadius: '0.75rem' }}>
              <label className="flex items-start cursor-pointer" style={{ gap: '1rem' }}>
                <input
                  ...
                  style={{ width: '1.25rem', height: '1.25rem', borderRadius: '0.25rem', marginTop: '0.125rem' }}
                  disabled={isSubmitting}
                />
                <div>
                  <span className="font-semibold text-gray-900 block" style={{ fontSize: '1rem', marginBottom: '0.25rem' }}>
                    Post anonymously
                  </span>
                  <p className="text-gray-600" style={{ fontSize: '0.875rem' }}>
                    Your identity will be hidden from others. Only you will know who posted this question.
                  </p>
                </div>
              </label>
            </div>
          ...
          {/* Action Buttons */}
          <div className="flex flex-wrap items-center border-t border-gray-100" style={{ gap: '0.75rem', paddingTop: '2rem' }}>
            <button
              type="submit"
              disabled={isSubmitting}
              ...
              style={{ gap: '0.5rem', padding: '0.875rem 2rem', borderRadius: '0.75rem', fontSize: '1rem' }}
            >
              {isSubmitting ? (
                <>
                  <div className="border-2 border-white border-t-transparent animate-spin" style={{ width: '1.25rem', height: '1.25rem', borderRadius: '50%' }}></div>
