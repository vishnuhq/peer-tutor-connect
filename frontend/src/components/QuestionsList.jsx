...
import { MessageSquarePlus, CheckCircle, MessageCircle, ChevronRight, Search, X } from 'lucide-react';
...
const QuestionsList = () => {
  ...
  const fetchQuestions = async () => {
    try {
      setLoading(true);
      const response = await questionsApi.getQuestionsByCourse(courseId, sortOption);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50/30">
      ...
      <main id="main-content" className="container-centered fade-in" style={{ paddingTop: '2rem', paddingBottom: '2rem', maxWidth: '64rem' }}>
        ...
        {/* Page Header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between" style={{ gap: '1rem', marginBottom: '2rem' }}>
          <div>
            <h1 className="font-bold text-gray-900 text-2xl sm:text-4xl" style={{ lineHeight: '1.2' }}>
              {course ? `${course.courseCode} - ${course.courseName}` : 'Loading...'}
            </h1>
            <p className="text-gray-600 text-base sm:text-lg" style={{ marginTop: '0.5rem' }}>
          ...
          <button
            ...
            style={{ gap: '0.5rem', padding: '1rem 1.5rem', borderRadius: '0.75rem', fontSize: '1rem' }}
            aria-label="Post a new question"
          >
            <MessageSquarePlus style={{ width: '1.25rem', height: '1.25rem' }} />
            Post Question
          </button>
        </div>

        {/* Compact Filter/Search/Sort Bar */}
        <div className="bg-white border border-gray-200 shadow-sm" style={{ borderRadius: '0.75rem', padding: '1rem', marginBottom: '1.5rem' }}>
          <div className="flex items-center flex-wrap" style={{ gap: '0.75rem' }}>
            ...
            <div className="flex items-center" style={{ gap: '0.375rem' }}>
              <button
                ...
                style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem', fontSize: '0.8125rem', whiteSpace: 'nowrap' }}
              >
                All
              </button>
              <button
            ...
            <div className="relative" style={{ flex: 1, minWidth: '200px' }}>
              <Search className="absolute text-gray-400" style={{ left: '0.75rem', top: '50%', transform: 'translateY(-50%)', width: '1.125rem', height: '1.125rem', pointerEvents: 'none' }} />
              <input
                type="text"
                ...
                style={{ padding: '0.5rem 2.5rem 0.5rem 2.5rem', borderRadius: '0.5rem', fontSize: '0.875rem' }}
              />
              {searchQuery && (
                <button
                  onClick={clearSearch}
            ...
            <div className="flex items-center" style={{ gap: '0.5rem' }}>
              <label htmlFor="sort" className="text-gray-700 font-semibold" style={{ fontSize: '0.875rem', whiteSpace: 'nowrap' }}>
                Sort by:
              </label>
              <select
                id="sort"
        ...
        {/* Questions List */}
        {loading ? (
          <div className="flex justify-center" style={{ paddingTop: '4rem', paddingBottom: '4rem' }}>
            <Spinner size="lg" text="Loading questions..." />
          </div>
        ) : questions.length === 0 ? (
          <div className="empty-state">
            <div className="bg-white shadow-lg mx-auto" style={{ padding: '3rem', borderRadius: '1rem', maxWidth: '28rem' }}>
              <MessageCircle className="text-gray-400 mx-auto" style={{ width: '5rem', height: '5rem', marginBottom: '1rem' }} />
              <h3 className="font-semibold text-gray-900" style={{ fontSize: '1.25rem', marginBottom: '0.5rem' }}>
                No questions yet
              </h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {questions.map((question) => (
              ...
              >
                ...
                <div className="flex items-start" style={{ gap: '0.75rem', marginBottom: '0.5rem' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div className="flex items-center" style={{ gap: '0.5rem', marginBottom: '0.375rem' }}>
                      <h3 className="font-bold text-gray-900 group-hover:text-teal-600 transition-colors line-clamp-1" style={{ fontSize: '1rem' }}>
                        {question.title}
                      </h3>
                      {/* Resolved Badge */}
                      {question.isResolved && (
                        <div className="flex-shrink-0">
                          <div className="flex items-center bg-green-100 text-green-700 font-semibold" style={{ gap: '0.25rem', padding: '0.125rem 0.5rem', borderRadius: '0.375rem', fontSize: '0.75rem' }}>
                    ...
                    <p className="text-gray-600 line-clamp-1 leading-normal" style={{ fontSize: '0.875rem' }}>
                      {question.content}
                    </p>
                  </div>
                </div>
                ...
                {/* Question Meta */}
                <div className="flex items-center text-gray-600" style={{ gap: '0.75rem', fontSize: '0.8125rem' }}>
                  <div className="flex items-center" style={{ gap: '0.375rem' }}>
                    <div className="bg-gradient-to-br from-teal-600 to-emerald-600 flex items-center justify-center" style={{ width: '1.5rem', height: '1.5rem', borderRadius: '50%' }}>
                      <span className="text-white font-bold" style={{ fontSize: '0.625rem' }}>
                  ...
                  {question.responseCount > 0 && (
                    <>
                      <span className="text-gray-400">•</span>
                      <span className="flex items-center font-medium text-teal-600" style={{ gap: '0.25rem' }}>
