...
const ResponseForm = ({
  ...
  return (
    <div className="border border-gray-100 bg-gradient-to-r from-teal-50 to-emerald-50 shadow-sm" style={{ borderRadius: '1rem', padding: '1.5rem' }}>
      <form onSubmit={handleSubmit}>
        {/* Content Field */}
        <div style={{ marginBottom: '1.25rem' }}>
          <div className="flex items-center justify-between" style={{ marginBottom: '0.75rem' }}>
            <label htmlFor="response-content" className="block font-semibold text-gray-900" style={{ fontSize: '1rem' }}>
              {isEdit ? 'Edit Response' : 'Your Response'} *
            </label>
            <span className={`font-medium ${content.length > 1500 ? 'text-red-600' : 'text-gray-600'}`} style={{ fontSize: '0.875rem' }}>
              {content.length}/1500
            </span>
          </div>
          <textarea
          ...
          <p className="text-gray-600 font-medium" style={{ marginTop: '0.5rem', fontSize: '0.75rem' }}>
            💡 Tip: Press <kbd className="bg-white border border-gray-300" style={{ padding: '0.125rem 0.5rem', borderRadius: '0.25rem', fontSize: '0.75rem' }}>Ctrl+Enter</kbd> to submit quickly
          </p>
        </div>

        {/* Anonymous Toggle (only for new responses) */}
        {!isEdit && (
          <div className="bg-white border border-teal-100" style={{ marginBottom: '1.25rem', padding: '1rem', borderRadius: '0.75rem' }}>
            <label className="flex items-start cursor-pointer" style={{ gap: '0.75rem' }}>
              <input
                type="checkbox"
                checked={isAnonymous}
              ...
              />
              <div>
                <span className="font-semibold text-gray-900 block" style={{ fontSize: '0.875rem' }}>
                  Post anonymously
                </span>
                <p className="text-gray-600" style={{ fontSize: '0.75rem', marginTop: '0.125rem' }}>
        ...
        <div className="flex flex-wrap items-center" style={{ gap: '0.75rem' }}>
          <button
            ...
            style={{ gap: '0.5rem', padding: '0.625rem 1.5rem', borderRadius: '0.75rem', fontSize: '0.9375rem' }}
          >
            {isSubmitting ? (
              <>
